...
@Component({
  selector: 'page-forum-show',
  templateUrl: 'forum-show.html'
})
export class ForumShowPage {
  ...
  transformForumData(data): any {
    ...
    for (let i = 0; i < data.topics.length; i++) {
      ...
      switch (data.topics[i].type) {
        case ForumShowPage.TYPE_ANNOUNCE:
          oneTitle += '<span style="color:#F96E02">【公告】</span>';
          break;
        case ForumShowPage.TYPE_STICKY:
          oneTitle += '<span style="color:#F96E02">【置顶】</span>';
          break;
        case ForumShowPage.TYPE_DIGEST:
          oneTitle += '<span style="color:#F96E02">【精品】</span>';
          break;
        case ForumShowPage.TYPE_SOLDOUT:
...
*/
  ...
  sendNewTopic() {
    if (this.userSettings.user == null) {
      ...
      let nicE = new nicEditors.findEditor('niceTopicTextArea');
      ...
      loader.present().then(() => {
        this.jcxapi.sendNewTopic(this.forumId, this.subjectField, messageStr).then(data => {
          ...
          if (result.errorMessage != null) {
            ...
            this.forumPage = null;
            this.pagesMap.clear();
            this.jq('#newTopic').slideUp(); this.jq('#openNewTopic').slideDown();
